  ...
  if(is_rotate == true)
    {
        ...
        my_serial.write("Br30j");
           ...
           if(abs(start_angle-im->orientation.z) < 5)
           {
...
///////edit  29/03/2015 ////
	...
	{
		if(abs(speedR-speedL)<20)
		{		
		ROS_ERROR("bootsssss robot");	
		speedL = 30;
		speedR = 30;
		}
		if(is_slopeH)
		{
			if(abs(speedR-speedL)<40)
			{	
				ROS_ERROR("bootsssss robot2");		
				speedL = 30;
				speedR = 30;
			}
		}
...
void LaserCallback(const sensor_msgs::LaserScanConstPtr& data)
{
   ...
   ////////////////////////// Divide laser //////////////////////
      ...
      if(s4min < 37 && s5min < 35 && s6min < 35 && s7min < 37)
      {
          is_stop = true;
   ...
   command_pub.publish(ss);
      ...
      else if(is_slope == true && s4min < 80 && s5min < 80 && s6min < 80 && s7min < 80) // aviod center
      {
          is_stop = true;
...
//		  //  ros::Duration(0.0).sleep();
      ...
      else if(is_slopeH == true && s1min < 70 && s2min < 70 && s3min < 70 && s4min < 70) // aviod left
      	{
          ROS_INFO("rotate left");
          my_serial.write("Al40j");
          ros::Duration(0.01).sleep();
      ...
      else if(is_slopeH == true && s7min < 70 && s8min < 70 && s9min < 70 && s10min < 70) // aviod right
            {
                ROS_INFO("rotate right");
                my_serial.write("Ar40j");
                ros::Duration(0.01).sleep();
	...
	else if (is_roll30 == true && s8min < 50 && s9min<40 && s10min<35)
	    {
...
ROS_ERROR("boot Robot escape from roll");
		...
		{		
			...
			my_serial.write("Ar70j");
			ros::Duration(0.01).sleep();
			my_serial.write("Bl15j");
			}
			else {
			my_serial.write("Ar60j");
			ros::Duration(0.01).sleep();
...
else if (is_roll30 == true && s3min < 50 && s2min<40 && s1min<35)
	    {
		ROS_INFO("boot Robot escape from roll");
		is_stop = true;
...
ROS_ERROR("boot Robot escape from roll");
		...
		{	
			...
			{	
			ROS_ERROR("boot Robot escape from roll2");
			my_serial.write("Ar15j");
                	ros::Duration(0.01).sleep();	
			my_serial.write("Bl70j");
			}
			else
			{
			my_serial.write("Ar20j");
               		ros::Duration(0.01).sleep();	
			my_serial.write("Bl60j");
			}		
		}
		else
		{
